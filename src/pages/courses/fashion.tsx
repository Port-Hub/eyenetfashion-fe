import Fashion1 from "../../assets/fashion/Fashion_1.jpg"
import Fashion2 from "../../assets/fashion/Fashion_2.jpg"
import Fashion3 from "../../assets/fashion/Fashion_3.jpg"
import Fashion4 from "../../assets/fashion/Fashion_4.jpg"
import Fashion5 from "../../assets/fashion/Fashion_5.jpg"
import Fashion6 from "../../assets/fashion/Fashion_6.jpg"
import Fashion7 from "../../assets/fashion/Fashion_7.jpg"
import Fashion8 from "../../assets/fashion/Fashion_8.jpg"
import Fashion9 from "../../assets/fashion/Fashion_9.jpg"

const Fashion: (arg: any) => JSX.Element = () => {
    return (
        <>
            <h2>Courses Offered</h2>

            <b><span>FASHION DESIGNING</span></b>

            <h4>Introduction</h4>

            <img src={Fashion1} alt="eyenetfashion" />

            <p>
                        These days career in fashion designing 
                        is one of the most rewardful professions. 
                        It makes you prominent among the masses. 
                        It also gives satisfaction, artistic look 
                        and a lucrative career as well. Fashion 
                        Designing course enables you to enter 
                        into the vast field of fashion and to 
                        respond the challenges of designing for 
                        a better future. Our course specially 
                        trains you to become master in the same 
                        field. Fashion is the key of rendering ones's 
                        presentation of own personality and style. 
                        It is an inclination in its own.
            </p>

            <span>What is Fashion Designing?</span>

            <p>
                        Fashion Designing commences from the creativity 
                        of the mind. It includes the taste, ideas, 
                        emotion and expression of an individual. 
	                    Selection of right fabric with suitable texture 
                        and converting it into an attractive dress, then 
                        it becomes suitable for specific people and 
                        specific season & occasion; these are all 
                        possible with the fashion designing techniques 
                        which has the proper mix of textile and fashion 
                        science. 
            </p>

            <span>Objectives:</span>

            <p>
                        Fashion Designing is the exclusive career for creative 
                        persons. To educate interested people in Fashion Designing, 
                        this course guides to develop their own designs with the 
                        current trends in fashion. The Fashion Designing course is 
                        completely differed from other tailoring courses in many ways.
                        This course will make ready the students with highest academic 
                        standard and practical knowledge, for at present what the 
                        companies and organizations need. In the competitive fashion 
                        world, Fashion Designing course makes the student &ldquo;ONE 
                        OUT OF THE BEST&rdquo;.
            </p>

            <span>How Fashion Designing differs from other Tailoring courses?</span>

            <p>
                        Fashion Designing course forms a student who has interest and 
                        creativity, with complete knowledge. It gives the exposure to 
                        the students in all its fields from the origin of fibres to the 
                        finished garment with all the Textile finishes.
            </p>
            <p>
	                    Generally Fashion Designing course forms a student according to 
                        the fulfillment of all the requirements of Textile & Garment 
                        Industries. After the completion of Fashion Designing Course
            </p>

            <div>
                <span>Job Opportunities</span>
                <ul>
                    <li>Coordinator in Export Houses</li>
                    <li>Apparel Merchandiser</li>
                    <li>Fashion Publishing</li>
                    <li>Quality Controller</li>
                    <li>Illustrator</li>
                    <li>Pattern Maker</li>
                    <li>Checker</li>
                    <li>Computer Aided Designer</li>
                    <li>Freelance Designer</li>
                    <li>Cine Costumer</li>
                    <li>Fashion Co-ordinator etc.</li>
                </ul>
	        </div>
	        <div>
	            <span>Business Opportunities</span>
	            <ul>
	                <li>Desinger Boutique Shops</li>
                    <li>Tailoring Boutique</li>
                    <li>Export and Import units</li>
                    <li>Tailoring units</li>
                    <li>Star trading houses</li>
                    <li>Fashion Complexes</li>
                    <li>Designer Studios</li>
                    <li>Computer Aided Designer</li>
                    <li>Fabrication Centers</li>
                    <li>Manufacturing Units & Textile Production houses</li>
                    <li>Fashion Consultant etc.</li>
	            </ul>
            </div>

            <h2>Diploma Courses</h2>

            <b><span>Advanced</span> Dip. in Fashion Designing</b>

            <div>
                <div>	
                    <h4>Advanced Diploma in Fashion Designing -- Duration 1 year</h4>

                    <span>Objective : </span>
                    <div>


                    <div>
                    <span>Semester 1</span>

                    <span>Design Ideas </span>
                    <ul>
                        <li> Types of necklines, sleeves, collars, waistlines etc. and their composition </li>
                        <li> Design ideas for kids like frocks, baba suit etc. </li>
                    </ul>

                    <span>Basic Design </span>
                    <ul>
                        <li> Elements of designs </li>
                        <li> composition of lines, shapes, colours etc. </li>
                        <li> simple concepts of prints & textures </li>
                    </ul>

                    <span>Fashion Illustration </span>
                    <ul>
                        <li> Basic human body anatomy </li>
                        <li> Different views, postures, turns & twists for female figures </li>
                        <li> Basic concepts of draping </li>
                    </ul>

                    <span>Textile Science </span>
                    <ul>
                        <li> Different kinds of Fibers & their properties </li>
                        <li> Fabric painting, block printing etc. </li>
                    </ul>

                    <span>Drafting</span>
                    <ul>
                        <li> Measurement technology, child bodice block, different sleeves, collars, frocks, baba suit etc. </li>
                    </ul>

                    <span>Pattern Making </span>
                    <ul>
                        <li> Construction patterns for said drafting </li>
                        <li> Pattern layout scheme to find out the consumption of fabric </li>
                    </ul>
                    <span>Constructions</span>
                    <ul>                        
                        <li>Sample constructions for machine-seams, tucks, pleats, necklines etc. </li>
                        <li> Garment manufacturing technologies for kids wear </li>
                    </ul>
                    <span>Management</span>
                    <ul>
                        <li> Terminology in fashion apparel market </li>
                        <li> Personality Development & Interviews preparation </li>
                        <li> Market Survey</li>
                    </ul> 
                </div>

                <div>
                    <b><span>Semester 2</span></b>
                    <span>Design Ideas </span>
                    <ul>
                        <li> Types of skirts, tops, silhouettes, cuffs etc. and their compositions </li>
                        <li> Design ideas for female wears  </li>
                    </ul>
                    <span>Basic Design </span>
                    <ul>
                        <li> Different colour schemes and their uses </li>
                        <li> Embroidery patterns </li>
                        <li> Enlargements & reductions </li>
                        <li> Uses of different textures  </li>
                    </ul>
                    <span>Fashion Illustration </span>
                    <ul>
                        <li> Different face features, hand style, foot style etc. </li>
                        <li> Uses of colour mediums in figures</li>
                        <li> Basic concepts of draping </li>
                        <li> Rendering techniques Stylized croquis etc.</li>
                        <li> Stylized croquis etc. </li>
                    </ul>
                    <span>Textile Science </span>
                    <ul>
                        <li>Yarns properties & formations of yarns </li>
                        <li> Different weaves & fabrics identification </li>
                        <li> Tie & dye, screen printing etc. </li>
                    </ul>

                    <span>Drafting</span>
                    <ul>
                        <li>Aprons, skirt-top, night wears, salwar suits, blouses, female trousers etc.  </li>
                    </ul>

                    <span>Pattern Making </span>
                    <ul>
                        <li>Construction patterns and layout scheme </li>
                        <li> Dart manipulation technology (single dart series)</li>
                    </ul>

                    <span>Constructions</span>
                    <ul>
                        <li>Garment manufacturing technology for female wears</li>
                        <li> Uses of different decorative stitches</li>
                    </ul>
                    <span>Management</span>
                    <ul>
                        <li>Nature of fashion & fashion market </li>
                        <li> Fashion forecasting & prediction </li>
                    </ul> 
                </div>
            </div>
            <b><span>Diploma</span> in Dress Making (Female)</b>
            <div>
                <div>
                    <h4>Diploma in Dress Making (Female) -- Duration 6 Months</h4>
                    <span>Objective : </span>
                    <p>
                        Fashion Designing is no longer domain and 
                        essential for women only.  Masses have 
                        realized the merits of looking good with 
                        customized attires. With wide range of 
                        Indian Apparel available, one can make a 
                        choice of &ldquo;looking good&rdquo; and 
                        &ldquo;looking the best&rdquo;. Therefore 
                        this course is essentially designed to groom 
                        your vibrant talent and creativeness to not 
                        only suit your self but many thronging the 
                        boutiques the world over.  Yes of course , 
                        Computer Aided Fashion Designing is a new 
                        dimension, a &ldquo;value addition&rdquo; 
                        to your potent &ldquo;Market value&rdquo;.  
                        After completion of this course you could be 
                        looking for a career as a Merchandiser or a 
                        Fashion co-ordinator or a Sampling Manager, 
                        to name a few.
                    </p>
                    <span>Design ideas:</span>
                    <ul>
                        <li>Types of necklines</li>
                        <li>Sleeves</li>
                        <li>Collars</li>
                        <li>Waistlines etc and their composition</li>
                        <li>Types of skirts</li>
                        <li>Tops</li>
                        <li>Sihouettes</li>
                        <li>Cuffs etc and their compositions</li>
                        <li>Design ideas for female wear</li>
                    </ul>
                    <span>Drafting & Pattern making techniques:</span>
                    <ul>
                        <li>Measurement techniques for all kinds of human figures.</li>
                        <li>Female wear &ndash; Pattern construction.</li>
                        <li>Pattern layout scheme for finding out the fabric consumption.</li>
                        <li>Pattern draping on pattern dummy.</li>
                        <li>Construction patterns and layout scheme </li>
                        <li>Dart manipulation technology (single dart series) </li>
                        <li>Double & multiple dart series</li>
                        <li>Dart blending </li>
                        <li>Pattern draping on pattern dummy </li>
                    </ul>
                    <span>Sample Constructions for&hellip;.</span>
                    <ul>
                        <li>Basic hand stitches</li>
                        <li>Samples for machine seams, fullness etc.</li>
                        <li>Different decorative stitches.</li>
                        <li>Garment Mfg Technology for female wear.</li>
                    </ul>
                </div>
            </div>
            <b><span>Diploma</span>&nbsp;in Dress Making (Kids)</b>
            <div>	
                <div>
                    <h4>Diploma in Dress making (Kids) -- Duration 6 Months</h4>
                    <span>Design ideas:</span>
                    <ul>
                        <li>Types of necklines</li>
                        <li>Sleeves</li>
                        <li>Collars</li>
                        <li>Waistlines etc and their composition</li>
                        <li>Types of skirts</li>
                        <li>Tops</li>
                        <li>Sihouettes</li>
                        <li>Cuffs etc and their compositions</li>
                        <li>Design ideas for kids wear</li>
                    </ul>
                    <span>Drafting & Pattern making techniques:</span>
                    <ul>
                        <li>Measurement techniques figures.</li>
                        <li>kids wear &ndash; Pattern construction.</li>
                        <li>Pattern layout scheme for finding out the fabric consumption.</li>
                        <li>Construction patterns and layout scheme </li>
                        <li>Dart manipulation technology (single dart series) </li>
                        <li>Dart blending </li>
                        <li>Pattern draping on pattern dummy  </li>
                    </ul>
                    <span>Sample Constructions for&hellip;.</span>
                    <ul>
                        <li>Basic hand stitches</li>
                        <li>Samples for machine seams, fullness etc.</li>
                        <li>Different decorative stitches.</li>
                        <li>Garment Mfg Technology for kids wear.</li>
                    </ul>
                </div>
            </div>
            <b><span>Diploma</span>&nbsp;in Tailoring (Advance)</b>
            <div>	
                <div>
                    <h4>Diploma in Taloring (Advance) -- Duration </h4>
                    <span>Tailoring Course</span>
                    <p>This course is designed to teach the various nuances 
                        of pattern making and Tailoring in a comprehensive 
                        manner. The patterns cater to both standard measurements 
                        & personal . By learning different types of patterns, 
                        a student would be able to stylishly design her own 
                        dress according to the latest  trends. This course is 
                        useful for persons who wish to start their own business 
                        such as establishing a tailoring unit or a Boutique 
                        as well as for Professional garment design opportunities 
                        in the Industry.
                    </p>
                <span>Course Topics </span>
                <ul>
                    <li>Sewing Basics</li>
                    <li>About Sewing Machine</li>
                    <li>About  Fabric</li>
                    <li>Basic Hand Stitches</li>
                    <li>Basic Machine Stitches</li>
                    <li>Basic Design Ideas</li>
                    <li>Measurement Techniques</li>
                    <li>Drafting</li>
                    <li>Pattern Making</li>
                    <li>Garment Construction (Female/Kids)</li>
                </ul>
            </div>
        </div>
        <h2>Certificate Courses</h2>
        <b><span>Chudhidhar</span> Making</b>
        <div>
            <div>
                <h4>Chudhidhar Making -- Duration 2 Months</h4>
                <img src={Fashion2} alt="eyenetfashion"/>
                <span>Objectives</span>
                <p>
                    Churidars are among the most common outfits for 
                    the women in India. They are suitable for all age 
                    groups. In this course we teach the students about 
                    different styles of churidars and how to fit them 
                    to different body structured women in attractive 
                    manner.
                </p>
                <div>
                    <span>Tops</span>
                    <ul>
                        <li>Chudi-basic</li>
                        <li>Chudi with dart</li>
                        <li>Princess</li>
                        <li>Style line</li>
                        <li>Hi-neck</li>
                        <li>Hi-neck with collar</li>
                        <li>Umbrella</li>
                        <li>Anarkali</li>
                        <li>Layered</li>
                        <li>Novelty</li>
                        <li>Panel</li>
                        <li>Magyar type</li>
                    </ul>
                    <span>Neck Lines</span>
                    <ul>
                        <li>Basic lines</li>
                        <li>Style lines</li>
                    </ul>
                </div>
                <div>
                    <span>Pant</span>
                    <ul>
                        <li>Basic</li>
                        <li>Gathering</li>
                        <li>Sal war</li>
                        <li>Patiala</li>
                        <li>Dhoti</li>
                        <li>Parallel</li>
                        <li>Palazzo</li>
                    </ul>
                    <span>Sleeves</span>
                    <ul>
                        <li>Basic (3 types)</li>
                        <li>Puff sleeve (3 types)</li>
                        <li>Bell sleeve</li>
                        <li>Petal sleeve</li>
                    </ul>
                </div>
            </div>
        </div>
        <b><span>Blouse</span>&nbsp;Making</b>
        <div >
            <div>
                <h4>Blouse Making -- Duration 2 Months</h4>
                <img src={Fashion3} alt="eyenetfashion" />
            </div>
            <span>objectives</span>
            <p>
                Only the perfect fit gives good look to any garments. 
                Especially a well fitted blouse accentuates saree's 
                appeal.  By this course, the students can come to know 
                about measurements of a specific types of blouse and 
                how to make a fit as perfect one.
            </p>
            <div>
                <span>Tops</span>
                <ul>
                    <li>Basic</li>
                    <li>4-dart</li>
                    <li>4-dart with belt</li>
                    <li>Princess</li>
                    <li>Katori</li>
                    <li>Hi-neck</li>
                    <li>Hi-neck with collar</li>
                    <li>Asymmetrical cutting</li>
                </ul>
            </div>
            <div>
                <span>Sleeves</span>
                <ul>
                    <li>Basic ( 3 types)</li>
                    <li>Puff sleeve ( 3 types)</li>
                    <li>Bell sleeve</li>
                    <li>Petal sleeve</li>
                </ul>
                <span>Neck Lines</span>
                <ul>
                    <li>Basic lines</li>
                    <li>Style lines</li>
                </ul>
            </div>
        </div>
        <b><span>Drafting</span>&Pattern Making</b>
        <div>	
            <div>
                <h4>Pattern Making -- Duration </h4>
                <img src={Fashion4} alt="eyenetfashion" />
                <span>Objective:</span>
                <div>
                    <p>
                        In this course, students learn to develop basic 
                        body slopers using standard specification measurements. 
                        Collars and basic sleeve shapes are made. Paper patterns,
                        as well as a final fabric sample, are developed. 
                        Students learn the importance of fit and balance in 
                        order to create a high-quality production pattern. 
                        Specifications are discussed in relation to both pattern 
                        development and finished garment measurements.
                    </p>
                    <p>
                        Students learn the basic draping methods of developing 
                        production patterns for the garment industry.
                    </p>
                    <span>Course Contents:</span>
                    <span>Drafting & Pattern making techniques:</span>
                    <ul>
                        <li>Measurement techniques figures.</li>
                        <li>Drafting</li>
                        <li>Pattern construction.</li>
                        <li>Pattern layout scheme for finding out the fabric consumption.</li>
                        <li>Construction patterns and layout scheme</li>
                        <li>Pattern grading</li>
                        <li>Dart manipulation technology </li>
                        <li>Dart blending</li>
                        <li>Pattern draping on pattern dummy</li>
                    </ul>
                </div>
            </div>
        </div>
        <b><span>Aari</span> Embroidery</b>
        <div>
            <div>
                <h4>Aari Embroidery -- Duration 2 Months</h4>
                <img src={Fashion5} alt="eyenetfashion" />
                <span>Objectives</span>
                <p>
                Aari work has always been considered one of the most tedious 
                forms of needle work.  Today, with the help of advanced stitching 
                procedures and the role of more artisans, it takes lesser time to 
                complete the embroidery work on an outfit.  Training will be given 
                in Round Iron Stand with wooden frame. Kameez Neck Designs, Bridal 
                Blouse Designs including Back neck, Front neck, Sleeves marking for 
                Embroidery and etc.
            </p>
            <div>
                <span>Syllabus</span>
                <ul>
                    <li>Running Chain</li>
                    <li>Zig Zag Chain</li>
                    <li>Scroll Chain</li>
                    <li>Magic Chain</li>
                    <li>Satin Stitch</li>
                    <li>Stem Stitch</li>
                    <li>Over Casting</li>
                    <li>Long & Short</li>
                    <li>Leaf Stitch</li>
                    <li>Salli Stitch</li>
                    <li>Double Salli</li>
                    <li>Jigu Jigu work</li>
                    <li>Reverse Chain</li>
                </ul>
            </div>
            <div>
                <span></span>
                <ul>
                    <li>French knot</li>
                    <li>Zardosi</li>
                    <li>Paded Zardosi Work</li>
                    <li>Chumki Work</li>
                    <li>French Knot with Chumki</li>
                    <li>Load Work</li>
                    <li>Patch Work</li>
                    <li>Cut Work</li>
                    <li>Bead Work</li>
                    <li>Thambur Stitch</li>
                    <li>Stone work</li>
                    <li>Mirror Work</li>
                    <li>Kundan Work</li>
                </ul>
            </div>
        </div>
    </div>
    <h2>Short Term Courses</h2>
    <b><span>Illustration</span></b>
    <div>
        <div>
            <img src={Fashion6} alt="eyenetfashion" />
            <h4>Illustration -- Duration </h4>
            <p>
                The role of a fashion illustrator is to make the people aware 
                of the latest trends in fashion. A fashion illustrator does this 
                by means of his or her drawings. He or she works as a link between 
                the fashion industry and the people who are interested in fashion. 
                Fashions illustrations India is a well established service you can 
                avail to get cost effective service.
            </p>
            <p>
                Fashion illustration courses teach students to create images of 
                apparel, shoes and accessories for fashion advertising campaigns, 
                pattern books, catalogs, magazines and other fashion media. 
                Coursework emphasizes artistic skills, such as proportion, color, 
                texture and composition.
            </p>
            <span>Course Contents:</span>
            <ul>
                <li>Body Anatomy</li>
                <li>Block Figure</li>
                <li>Stick Figure </li>
                <li>Stylized female Croquis </li>
                <li>Fleshing out the shape</li>
                <li>Different postures, turns & twists for female figures</li>
                <li>Colour Theory</li>
                <li>Uses of colour medium in figures</li>
                <li>Fashion Accessories</li>
            </ul>
        </div>
    </div>
    <b><span>Fabric</span>&nbsp;Paintings</b>
    <div>
        <div>
            <h4>Fabric Paintings -- Duration 2 Months</h4>
            <img src={Fashion7} alt="eyenetfashion" />
            <span>Objective:</span>
            <p>
                Fabric paint is a type of paint which is specifically designed 
                for use on textiles. It is often used to create custom craft projects, 
                and is also used on a commercial scale to apply patterns and decorations 
                to textiles. Fabric Painting courses teach students to create images 
                through the process of fabric painting, different fabrics such as 
                apparel fabrics or home furnishing fabrics can be given a crafty look. 
                In fact, it can also be used for embellishing finished garments such as 
                t-shirt, pants, jeans, and jackets as also many other products like bags, 
                pillows, or anything made up of fabrics just by adding designs and 
                colors to them.
            </p>
            <div>
                <span>Design Ideas</span>
                <ul>
                    <li>Colour Theory</li>
                    <li>Design Developing</li> 
                    <li>Basic strokes</li>
                    <li>Plain filling work</li>
                    <li>Shading work</li>
                    <li>Dry Shading</li>
                    <li>Tapping method</li>
                    <li>Inwards and Outward strokes</li>
                    <li>Stencil painting</li>
                </ul>
            </div>
            <div>
                <span>Transferring Ideas</span>
                <ul>
                    <li>Direct  Method</li>
                    <li>Carbon tracing	</li>
                    <li>Chalk powder tracing</li>
                </ul>
            </div>
            <div>
                <span>Type of Medium</span>
                <ul>
                    <li>Classic Colours</li>
                    <li>Pearl Colours</li>
                    <li>Silk Colours</li>
                    <li>Fabric  Out Liners</li>
                </ul>
            </div>
        </div>
    </div>
    <b><span>Embroidery </span>& Hand Works</b>
    <div>	
        <div>
            <h4>Embroidery & Hand Works -- Duration </h4>
            <img src={Fashion8} alt="eyenetfashion" />
            <span>Objective:</span>
            <p>
                To train the students better way to implement their embroidery 
                talents because the embroidery is a beautiful thread work on a 
                variety of fabrics, which make the fabric more attractive.  
                Women are the pioneers in creating excellent delicate embroidered 
                garments since ancient times. The taste and requirement for 
                embroidered garments in the modern society is in increasing demand 
                due to the fast changing fashion and their gracious look.  
                Some of the traditional embroideries need to be popularized 
                and developed into an industrial craft in order to save the 
                reminiscent beauty of the past and ever lasting art. 
            </p>
            <div>
                <h5>Design Ideas</h5>
                <ul>
                    <li>Colour Theory</li>
                    <li>Design Developing</li> 
                </ul>
                <h5>Type of Threads</h5>
                <ul>
                    <li>Silk thread</li>
                    <li>Hand embroidery thread</li>
                    <li>Ribbon</li>
                </ul>
                <span>Type of Stitches</span>
                <h5>Straight Family</h5>
                <ul>
                    <li>Running stitch</li>
                    <li>Stem stitch</li>
                    <li>Double stem stitch</li>
                </ul>
                <h5>Round Family</h5>
                <ul>
                    <li>Chickenkari</li>
                    <li>Eye let</li>
                    <li>Spider web</li>
                    <li>Back spider web</li>
                    <li>Round buttonhole</li>
                    <li>Coin stitch</li>
                </ul>
                <h5>Crossed Family</h5>
                <ul>
                    <li>Cross stitch</li>
                    <li>Herringbone</li>
                    <li>Double herringbone</li>
                    <li>Chevron stitch</li>
                    <li>Fly stitch</li>
                    <li>Feather stitch</li>
                    <li>Vandyke stitch</li>
                    <li>Star filling</li>
                </ul>
                <h5>Filling</h5>
                <ul>
                    <li>Satin stitch</li>
                    <li>Encroaching satin</li>
                    <li>Long & short filling</li>
                    <li>Laid(Darning)</li>
                    <li>Chain filling</li>
                    <li>Blanket filling</li>
                </ul>
                <h5>Indian Embroidery</h5>
                <ul>
                    <li>Mirror work</li>
                    <li>Shadow work</li>
                    <li>Sequence work</li>
                    <li>Appliqu&eacute; work</li>
                    <li>Net appliqu&eacute;</li>
                    <li>Jardhosi & Bead work</li>
                </ul>
            </div>
            <div>
                <h5>Transferring Ideas</h5>
                <ul>
                    <li>Direct Method</li>
                    <li>Carbon tracing</li>
                    <li>Chalk powder tracing</li>
                </ul>
                <h5>Chain Family</h5>
                <ul>
                    <li>Simple chain stitch</li>
                    <li>Broken chain stitch </li>
                    <li>Alternate&nbsp;(magic)chain&nbsp;stitch </li>
                    <li>Whipped chain stitch</li>
                    <li>Raised chain</li>
                    <li>Cable chain</li>
                    <li>Open chain</li>
                    <li>Zig zag chain</li>
                    <li>Splite chain</li>
                    <li>Twisted chain</li>
                </ul>
                <h5>Blanket Family</h5>
                <ul>
                    <li>Blanket stitch</li>
                    <li>Long & short blanket stitch</li>
                    <li>Group blanket stitch</li>
                    <li>Un even blanket </li>
                    <li>sequence blanket stitch </li>
                    <li>Closed blanket stitch</li>
                    <li>Cross blanket stitch </li>
                    <li>Button hole stitch</li>
                </ul>
                <h5>Whipping and Couching</h5>
                <ul>
                    <li>Spiral whipping</li>
                    <li>Pekience whipping</li>
                    <li>Cross whipping</li>
                    <li>Blanket whipping</li>
                    <li>Cloud filling</li>
                    <li>Simple couching</li>
                    <li>Couch Telles</li>
                </ul>
                <h5>Knot and Dart Family</h5>
                <ul>
                    <li>German knot</li>
                    <li>French knot</li>
                    <li>Bullion knot</li>
                    <li>Lazy-Dazy stitch</li>
                    <li>Dot stitch</li>
                    <li>Double lazy-dazy</li>
                    <li>Roumanian knot</li>
                </ul>
            </div>
        </div>
    </div>
    <b><span>Jewellery </span> Making</b>
    <div>	
        <div>
            <h4>Jewellery Making -- Duration </h4>
            <img src={Fashion9} alt="eyenetfashion"/>
            <span>Objective:</span>
            <p>
                The course is recommended for anyone who is into the family 
                business of jewelry and wants to sharpen his/her skill or an 
                individual who is interested in joining the business of jewelry, 
                an industry that is seeing a rapid growth in India as well as 
                in the international market. The course offers a launch pad for 
                creative minds to learn new skills and techniques in the highly 
                exciting and creative field of jewelry making.
            </p>
            <p>
                Costume jewelry is often 'mixed media', meaning that it's made 
                from a combination of different materials such as metal, clay, 
                fabric, plastic etc. Costume jewelry making is accessible to everyone 
                and is a popular and fun hobby. To make your own jewelry means that 
                you can keep up-to-date with fashion trends for much less money, 
                plus you can make accessories that are completely unique to you. If 
                you see a necklace, bracelet or pair of earrings you like in a high 
                street shop then you can use it for inspiration to make your own 
                homemade version.
            </p>
            <span>Course Contents</span>
            <ul>
                <li>Kundan jewelry </li>
                <li>Beaded jewelry</li>
                <li>Paper jewelry </li>
                <li>Paper quilling </li>
                <li>Clay jewelry (Shilpakar)</li>
                <li>Terracotta jewelry</li>
                <li>Ribbon jewelry </li>
                <li>Threaded Ornaments</li>
            </ul>
        </div>
    </div>
</div>
</div>
</>
    )
}

export default Fashion;